...
LinkedList* llist_initialize(int typeSize, char* typeName)
{
	...
	// Allocate enough data to hold the type name.
	...
	// Copy the type name into the struct's type string.
...
bool llist_add_at(LinkedList* llist, int index, void* element)
{
	...
	// If we can't allocate memory, return false.
	if(node == NULL)
		return false;
	...
	// Copy the element data into the node.
	...
	// We have successfully added to the list. Return true.
	return true;
}
...
void* llist_get(LinkedList* llist, int index)
{
	...
	// Allocate enough space for a copy of the data.
	void* data = malloc(llist->itemSize);

	// If we can't allocate space, return null.
	if(data == NULL)
		return NULL;

	// Copy the memory into our new memory space.
	memcpy(data, temp->data, llist->itemSize);
...
void* llist_remove(LinkedList* llist, int index)
{
	...
	// Return the removed data. This should be the only
	// pointer to it. It is up to the owner to free it.
	return data;
}
